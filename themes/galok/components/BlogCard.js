...
const BlogCard = ({ showAnimate, post, showSummary }) => {
  ...
  const showPreview =
    siteConfig('GALOK_POST_LIST_PREVIEW', null, CONFIG) && post.blockMap
  // galok 强制显示图片
  if (
    siteConfig('GALOK_POST_LIST_COVER_FORCE', null, CONFIG) &&
    post &&
    !post.pageCover
  ) {
  ...
  const showPageCover =
    siteConfig('GALOK_POST_LIST_COVER', null, CONFIG) &&
    post?.pageCoverThumbnail

  const GALOK_POST_LIST_ANIMATION =
    siteConfig('GALOK_POST_LIST_ANIMATION', null, CONFIG) || showAnimate

  // 动画样式  首屏卡片不用，后面翻出来的加动画
  const aosProps = GALOK_POST_LIST_ANIMATION
    ? {
        'data-aos': 'fade-up',
  ...
  return (
    <article
      ...
      className='w-full lg:max-w-sm p-3 mb-6 mx-2 bg-white dark:bg-hexo-black-gray 
             rounded-lg:shadow-md shadow-lg hover:shadow-xl 
             rounded-xl transition-all duration-300 
             transform hover:-translate-y-1 
             border border-gray-100 dark:border-gray-800 glass-effect'>
